...
| `query_type` | How to pass arguments to `condor_history` queries. `shell` gives full shell featues but requires escaping shell metacharacters and spaces. `exec` uses all `key`s literally. |
