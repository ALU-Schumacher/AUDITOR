...
function start_auditor() {
  if [ "$RELEASE_MODE" = true ]; then
    AUDITOR_APPLICATION__ADDR=0.0.0.0 ./target/release/auditor auditor/configuration/rbac.yaml &
  else
    AUDITOR_APPLICATION__ADDR=0.0.0.0 ./target/debug/auditor auditor/configuration/rbac.yaml &
  fi
  AUDITOR_SERVER_PID=$!
  COUNTER=0
